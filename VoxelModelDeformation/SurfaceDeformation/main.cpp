...
int surface_deformation(std::string meshfilename_name, std::string control_file, std::string transformation_file)
{

  Polyhedron mesh;
  std::ifstream input(meshfilename_name);
  ...
  input.open(transformation_file);
  ...
  std::ofstream output("deform_res.stl");
  output << mesh;
  output.close();
}
